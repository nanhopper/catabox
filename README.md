...
The two Leaving soon collections are current, platform-specific announcements. The official Xbox website identifies affected games but does not reliably publish an exact removal date. When the underlying collections provide an explicit date, Catabox shows it in the game badge and update summary; otherwise, the badge says that the date is not published. Catabox does not infer one from DisplayCatalog offer `EndDate` values, which are undocumented and can describe purchase, trial, pricing, delisting, or subscription windows rather than a Game Pass removal. A leaving-soon ID absent from the current tier catalog is reported as a warning and does not create a game.
